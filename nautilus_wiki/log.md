...
- 2026-08-22 · koşu e3271b87: fallback-kökenli aday (859cff3214dc, +%344 al-tut fazlası) köken kuralıyla robustluktan atlandı — üç koşunun tek alfa-geçeni. Tasarım sorusu açık: kural mı, model mi, etiketli tarama mı? → [[kapi_ucdan_uca_dogrulandi_2026_08_21]]
- 2026-08-22 · AUTO brief'e hazır prompt seçici: `web/auto_hint_presets.py` + `fragments/auto_hint_presets.html` (seçim doldurur, kilitlemez; Kaydet/Sil localStorage). Modül haritasına satır eklendi. → [[webapp_module_map]]
- 2026-08-22 · harici (noktalı) sembolde CATEGORY uygulanmıyordu ama etiket olarak kayıtlara sızıyordu (04e2dbff linear / e3271b87 spot → aynı NASDAQ·CASH motoru). Kapatıldı: rota `_effective_category` ile "" yazar, brief "—" gösterir, formda select pasif + not. Test: `tests/test_external_symbol_has_no_category.py`. → [[webapp_module_map]]
- 2026-08-22 · `strategy_studio/runner.py` sessiz-pending yarışı: düğüm daha 'running' denmeden çıkarsa hiçbir durum yazılmıyordu (flaky test 1/15 ile görünür oldu). 'running' artık kayıt kontrolüyle aynı kilit altında yazılıyor; erken çıkış bayrakla launch'a bildiriliyor → her yol bir durum yazar. Test deterministik (built.wait geciktirilerek serve thread'i kazanıyor), mutasyonla doğrulandı. → [[webapp_module_map]]
- 2026-08-22 · ÜRETİM: pipe_serve.py + node_proxy.js — Kaspersky'nin python.exe TCP-yanıt engelini named pipe ile atlayan üretim girişi; nautilus.muratben.com böyle açıldı. Modül haritasına eklendi. → [[webapp_module_map]]
