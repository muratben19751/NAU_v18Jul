...
| `strategies.py` | `MACrossoverStrategy` + `RSIMeanReversionStrategy` + **`VolTargetedTrendStrategy`** (MA crossover yönü + `calc_ewma_vol` ile dinamik pozisyon boyutlandırma; `allow_short=True` iken long+short, `STRATEGY_REGISTRY`/`STRATEGY_PARAM_SPEC` ile kayıtlı) | [[strategy_and_actor]] hiyerarşisi (Actor + Strategy + Config), [[v1_to_v2_migration_lessons]] StrategyConfig plain-class kalıbı, [[vol_targeted_trend]] |
