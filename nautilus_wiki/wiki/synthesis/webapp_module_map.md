...
| `web/routes/backtest.py` | **/backtest** birleşik tarif-odaklı panel: stratejiyi doğal dille tarif et → `POST /backtest/describe` Claude'a entry/exit için AYRI custom blok(lar) yazdırır (`propose_condition_breakdown` → koşul-başına `propose_custom_block` → blok-seviyesi OR/AND spec), sonra seçilen zaman dilimlerine göre zincirler: **2+ TF → `POST /backtest/sweep`** (aynı spec'i her TF'de **paralel** koşup —`ThreadPoolExecutor` + `force_subprocess`, `NAUTILUS_PARALLEL` kill-switch— karşılaştırma tablosu; cache-yok TF atlanır, en-iyi sütun yeşil; sweep başlığında ŞU AN koşan TF + `n/n` ilerleme), **1 TF → `POST /backtest/run`** (tam sonuç + equity + robustness OOB). **Canlı plan önizleme** (`POST /backtest/plan`): tarif yazılırken (debounce) tek `propose_condition_breakdown` çağrısıyla yorumlanır, koşullar YEREL heuristikle built-in bloklara eşlenir (`_match_builtin`/`_INDICATOR_TO_BUILTIN`), tuzaklar tahmin edilir (`_predict_plan_warnings`: allow_short kapalıyken short tarifi, 3+ AND→~0 işlem, size_precision=0 equity, Bollinger-cross), ve eşleşen built-in'lerin son ~400 BTC-1m barındaki al/sat/çıkış noktaları + indikatör overlay'leri (MA/EMA/Bollinger fiyat üstünde, RSI ayrı pane) `_preview_signals` ile tahmini gösterilir (LLM'siz, salt-okunur — hiçbir şey üretmez/kaydetmez). **allow_short toggle** formda (varsayılan AÇIK) → describe worker spec'e taşır (MARGIN hesap). **Canlı ilerleme flow'u**: `/backtest/progress/{run_id}` worker'ın serbest mesajlarını kanonik Nautilus fazlarına eşler (`_derive_bt_phases`/`_BT_PHASES`: Başlat→Veri→Aralık→Bar→Trend→Engine→Simülasyon→Topla→Metrik→Tamam) — tamamlanan yeşil, aktif faz yanıp söner, altında ham timestamp'li log. Sembol datalist (yaz-bul), çoklu-TF checkbox. `_normalize_intervals` ortak; `intervals_csv` hx-vals dizi kodlamasına güvenmeden çoklu-TF taşır. Index/External backtest route'ları (`/run`, `/tickers`, `/external_instruments`) korunur | [[backtesting_guide]], [[environment_contexts]], [[bar_aggregation_and_type_syntax]] |
