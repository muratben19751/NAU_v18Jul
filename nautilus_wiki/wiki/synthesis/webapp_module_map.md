...
- **Regresyon süiti**: **304 test** (pytest); NAU sabitleri, sizing modları, fold-kabul, codegate red-matrisi (subscript/dict/alias-callee bypass + dangerous-builtin referansı dahil), path-traversal guard, force_refresh-merge, delete-skip, flip yolu, per-trade-sharpe hizalaması, describe→sweep çoklu-TF zinciri (sıra-bağımsız), MTF trend filtresi ve LLM kredi-fallback dahil çapalar. Ruff lint/format temiz (PostToolUse hook).
