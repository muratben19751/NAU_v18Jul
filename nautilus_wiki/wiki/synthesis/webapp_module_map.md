...
| `strategies.py` | `MACrossoverStrategy` + `RSIMeanReversionStrategy` + **`VolTargetedTrendStrategy`** (MA crossover yönü + incremental EWMA vol ile dinamik pozisyon boyutlandırma; `allow_short=True` iken long+short, `STRATEGY_REGISTRY`/`STRATEGY_PARAM_SPEC` ile kayıtlı). **Perf (2026-07-19)**: tüm stratejilerde `_iid`/`_bt`/`_fixed_qty` `on_start`'ta cache'lenir (per-bar isinstance ortadan kalkar); `MACrossoverStrategy` + `VolTargetedTrendStrategy` `_fast_sum`/`_slow_sum` incremental running sum ile O(1) MA; `VolTargetedTrendStrategy` EWMA varyansı `self._ewma_var` ile O(1)/bar (artık her bar `calc_ewma_vol` tam buffer iterasyonu yok); `positions_open()` yalnız sinyal dalında çağrılır (her bar değil) | [[strategy_and_actor]] hiyerarşisi (Actor + Strategy + Config), [[v1_to_v2_migration_lessons]] StrategyConfig plain-class kalıbı, [[vol_targeted_trend]] |
