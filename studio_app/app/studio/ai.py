...
def build_prompt(defn: StrategyDefinition, metrics: BacktestMetrics | None,
                 ask: str, scope: str | None,
                 rejected: list[str], indicators: list[str]) -> str:
        ...
        try:
            ...
            if attempt == 1:
                raise SuggestionFailure(
                    f"LLM returned invalid suggestion: {e}") from e
            raw = client.complete(
...
def apply_suggestion(defn: StrategyDefinition, s: Suggestion) -> None:
    ...
    d = s.diff
    if s.kind == "add_rule":
        rule = add_rule(defn, s.block, d.get("indicator", ""),
                        bool(d.get("as_filter", False)))
        ...
        if owner == "risk":
            ...
            update_rule_param(defn, owner, d.get("param", ""),
                              str(d.get("value")))
...
def evaluate_trial(defn: StrategyDefinition, s: Suggestion,
                   adapter: BacktestAdapter,
                   baseline: BacktestMetrics | None,
                   min_trades: int) -> tuple[BacktestMetrics, StrategyDefinition]:
    ...
    if metrics.trades < min_trades:
        raise GuardrailReject(
            f"trades {metrics.trades} below guardrail {min_trades}")
    if baseline is not None and _objective(trial, metrics) < \
            _objective(trial, baseline):
        raise GuardrailReject(
...
def improved(defn: StrategyDefinition, trial: BacktestMetrics,
             baseline: BacktestMetrics | None) -> bool:
    if baseline is None:
        return True
