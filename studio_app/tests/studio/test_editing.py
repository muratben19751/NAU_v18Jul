...
def test_add_rule_creates_draft(client):
    r = client.post(f"/studio/{SID}/blocks/entry/rules",
                    data={"indicator": "rsi"})
...
def test_add_rule_as_filter(client):
    client.post(f"/studio/{SID}/blocks/exit/rules",
...
def test_edit_param_valid(client):
    ...
    wt = d.entry.rules[0]
    r = client.patch(f"/studio/{SID}/rules/{wt.id}",
                     data={"param": "n1", "value": "12"})
    assert r.status_code == 200
    d2 = _draft(client)
...
def test_edit_target(client):
    d, _ = client.store.working_copy(SID)
    fz = d.entry.rules[1]
    client.patch(f"/studio/{SID}/rules/{fz.id}",
                 data={"param": "target", "value": "-1.75"})
...
def test_risk_edit_and_bounds(client):
    client.patch(f"/studio/{SID}/risk",
                 data={"name": "take_profit_r", "value": "2.2"})
    assert _draft(client).risk.take_profit_r.value == 2.2
    r = client.patch(f"/studio/{SID}/risk",
                     data={"name": "risk_per_trade_pct", "value": "9"})
...
def test_draft_survives_reload_and_page_shows_it(client):
    d, _ = client.store.working_copy(SID)
    wt = d.entry.rules[0]
    client.patch(f"/studio/{SID}/rules/{wt.id}",
                 data={"param": "n1", "value": "8"})
