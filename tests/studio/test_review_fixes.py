...
def test_time_stop_rule_message_no_longer_points_at_a_dropped_field():
    ...
    defn.entry.rules.append(
        Rule(id="r-ts", indicator="time_stop", params={"bars": _p(24)}, operator="true")
    )
...
def test_unknown_rule_id_is_404_not_422(client):
    ...
    r = client.patch(
        f"/studio/{SID}/rules/deadbeefdead", data={"param": "len", "value": "9"}
    )
...
def test_known_rule_with_bad_value_is_still_422(client):
    ...
    r = client.patch(
        f"/studio/{SID}/rules/{rid}", data={"param": "len", "value": "abc"}
    )
...
def test_risk_endpoint_accepts_both_name_and_param(client):
    ...
    assert (
        client.patch(
            f"/studio/{SID}/risk", data={"name": "take_profit_r", "value": "1.9"}
        ).status_code
        == 200
    )
    assert (
        client.patch(
            f"/studio/{SID}/risk", data={"param": "take_profit_r", "value": "1.7"}
        ).status_code
        == 200
    )
    assert client.patch(f"/studio/{SID}/risk", data={"value": "1.5"}).status_code == 422
...
def test_embargo_drops_the_leading_bars_of_each_fold(monkeypatch):
    ...
    NautilusBacktestAdapter(bars_loader=lambda s, tf: bars).run(compile_strategy(defn))
...
def test_stored_equity_curve_is_downsampled_for_the_sparkline(monkeypatch):
    ...
    monkeypatch.setattr(
        sandbox, "run_backtest_guarded", lambda spec, bars, recipe=None, **kw: _R()
    )
    bars = pd.DataFrame(
        {
            "open": [1.0] * 50,
...
def test_every_rule_endpoint_reports_a_missing_rule_as_404(client):
    ...
    assert (
        client.patch(
            f"/studio/{SID}/rules/{missing}", data={"param": "len", "value": "9"}
        ).status_code
        == 404
    )
    ...
    assert (
        client.patch(
            f"/studio/{SID}/opt/toggle", data={"owner": missing, "param": "len"}
        ).status_code
        == 404
    )
    assert (
